...
#pragma config(Motor,  motor2,          intakeMotorL,  tmotorVexIQ, PIDControl, reversed, encoder)
#pragma config(Motor,  motor3,          catapultL,     tmotorVexIQ, PIDControl, reversed, encoder)
#pragma config(Motor,  motor4,          catapultR,     tmotorVexIQ, PIDControl, encoder)
#pragma config(Motor,  motor5,          intakeMotorR,  tmotorVexIQ, PIDControl, encoder)
...
void setGyroStable()
{
	...
	while (fGyroDriftRate > ACCEPTABLE_DRIFT_RANGE)
	{
		...
		//setTouchLEDColor(LED,colorRed); uncomment when led
		//setTouchLEDBlinkTime(LED, 14,6);
		//wait1Msec(3000);
		//uncomment when ledsetTouchLEDBlinkTime(LED, 0,1);
		wait1Msec(2000);

/*		setTouchLEDColor(LED,colorGreen); uncomment when led
...
*/
		...
		if (fGyroDriftRate < ACCEPTABLE_DRIFT_RANGE)
			//uncomment when led setTouchLEDColor(LED, colorGreen);
		else
			//uncomment when led setTouchLEDColor(LED, colorRed);

		//uncomment when led setTouchLEDBlinkTime(LED, 4, 4);
		wait1Msec(1);
		// uncomment when led setTouchLEDColor(LED,colorNone);
		//uncomment when led setTouchLEDBlinkTime(LED, 1, 0);
	}
...
void initialize()
{
	...
	// uncomment when led setTouchLEDColor(LED,colorYellow);
...
void intakeToggle()
{
	if(intakeOn == true)
	{
		setMotorSpeed(intakeMotorL, 100);
		setMotorSpeed(intakeMotorR, 100);
	}
	else
	{
		setMotorSpeed(intakeMotorL, 0);
		setMotorSpeed(intakeMotorR, 0);
	}
...
task main()
{
	...
	setGyroStable();
	//uncomment when led setTouchLEDColor(LED,colorBlue);
	waitUntil(getBumperValue(Bumper));
...
/*	strafePID(100, 60, 5, 2, 1, 3);
	...
	//uncomment when led setTouchLEDRGB(LED, 0, 255, 50);
