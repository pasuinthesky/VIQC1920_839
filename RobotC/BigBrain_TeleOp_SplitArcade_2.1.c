...
int iScoopPos[4] = { -150, 250, 800, 1050 }; //pick_cube, keep_ball/release cube, ready_to_scoop_ball, score_ball
...
void scooper_preset()
{
	...
	else if ( ( getJoystickValue (BtnFDown) == 1 ) )
	{
		...
		{//Release Cube
			setMotorTarget(scoopMotor,(turnNumber*ENCODER_UNIT_PER_SCOOP_ROUND)+iScoopPos[1]+30,100);
			wait1Msec( abs( iScoopPos[1] + 30 - iScoopPos[0] ) * MS_PER_ENCODER_UNIT );
		}
		else
		{//Pickup Cube
			overwriteDrive = true;
...
task main()
{
	...
	nBattLevel = BattLevel/10;
	displayCenteredBigTextLine(3, "%f", nBattLevel);
	if(nBattLevel<LOW_BATTERY){
		setTouchLEDRGB(LED,255, 0, 0);
		}else if(nBattLevel<=8.0){
		setTouchLEDRGB(LED,000, 255, 000);
	}
