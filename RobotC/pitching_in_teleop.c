...
void initialize()
{
	...
	setMotorBrakeMode(catapultMotor, motorHold);
}
...
task catapult()
{
	...
	while(true)
	{
		...
		{/*
			setMotorTarget(gateMotor, 0, 100);
			setMotorSpeed(intakeMotor, 0);
			setMotorSpeed(catapultMotor, -100);
			setMotorSpeed(catapultMotor, 0)
			setMotorTarget(gateMotor, -115, 100);*/
			setTouchLEDRGB(LED, 255, 255, 255);
			setMotorSpeed(catapultMotor, -100);
			wait1Msec(500);
			timeStamp = time1[T1];
		}
		//else if(getDistanceValue(sonar)< 90 && !getBumperValue(bumper) && time1[T1] - timeStamp > 500)
		else if(getDistanceValue(sonar)< 90 && time1[T1] - timeStamp > 500)
		{
			displayTextLine(3, "%f", getDistanceValue(sonar));
			setTouchLEDRGB(LED, 255, 0, 255);
			setMotorSpeed(catapultMotor, 0);
		}
		writeDebugStreamLine( "%f %f %f %f %f", getTimerValue(T1), timeStamp, getDistanceValue(sonar), getBumperValue(bumper), getMotorSpeed(catapultMotor) );
		wait1Msec(dt);
	}
}
