...
#pragma config(Motor,  motor11,         liftMotor,     tmotorVexIQ, PIDControl, encoder)
...
int iLiftLevel[LIFT_LEVELS] = {35, 570};

#define CLAW_POSITIONS 2
int iClawPosition[CLAW_POSITIONS] = {15, 50};
...
void claw_preset()
{
	...
	//iClawPosition[0] and [1] are claw closed and claw open respectively
	if (getJoystickValue(BtnLUp)==1)
	{
		if (! claw_working)
		{
			claw_working = true;
			if ( abs(getMotorEncoder(clawMotor) - iClawPosition[1]) < CLAW_DELTA )
			{
				setMotorTarget(clawMotor,iClawPosition[0],100);
				wait1Msec( abs( iClawPosition[0] - iClawPosition[1] ) * MS_PER_ENCODER_UNIT );
			}
			else
			{
				setMotorTarget(clawMotor,iClawPosition[1],100);
				wait1Msec( abs( iClawPosition[0] - iClawPosition[1] ) * MS_PER_ENCODER_UNIT );
			}
		}
	}
...
void lift_preset()
{
	...
	else if (getJoystickValue(BtnRDown)==1)
	{
		for (int i=LIFT_LEVELS-1;i>0;i=i-1)
		{
			if ( getMotorEncoder(liftMotor) > ( iLiftLevel[i-1] + ARM_DELTA ) )
			{
				setMotorTarget(liftMotor, 400,100);
				setMotorSpeed(BL, -50 );
				setMotorSpeed(BR, 50 );
				setMotorSpeed(FL, -50 );
				setMotorSpeed(FR, 50 );
...
task main()
{
	...
	setMotorTarget(clawMotor, iClawPosition[1], 60);
	wait1Msec(1000);
	...
	while(true)
	{
		...
		displayCenteredTextLine(3, "%d, %d", getGyroDegrees(gyro), desired_heading);
